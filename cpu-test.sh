...
CSV_FILE="${RESULTS_DIR}/phase2_concurrent_results_copy_codec.csv"
...
CONCURRENT_TESTS=(2 4 8 16 25 36)
...
setup() {
    ...
    if [[ $camera_count -lt 36 ]]; then
        error "Not enough cameras for concurrent tests (need 36, have $camera_count)"
        exit 1
    fi
    ...
    # Create CSV header
    echo "test_id,test_type,threads,preset,concurrent_streams,camera_url,avg_cpu_percent,avg_memory_mb,total_system_cpu,network_rx_mb,network_tx_mb,speed_ratio,avg_bitrate_kbps,duration_seconds,success,error_message" > "$CSV_FILE"
...
parse_ffmpeg_output() {
    ...
    # Extract average bitrate from libx264 encoder statistics (in kb/s)
    local avg_bitrate="0"
...
run_single_test() {
    ...
    local csv_line="${test_id}_${concurrent_id},concurrent,2,ultrafast,${concurrent_id},$(echo "$camera_url" | tr ',' ';'),${avg_cpu},${avg_memory},${system_cpu_usage},${rx_mb},${tx_mb},${speed_ratio},${avg_bitrate},${actual_duration},${success},${error_msg}"
...
main() {
    ...
    echo "Concurrent tests: ${CONCURRENT_TESTS[*]} streams"
